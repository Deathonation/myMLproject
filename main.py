...
def detect(frame):
    ...
    global totalperson
    print("bounding_box_cordinates",bounding_box_cordinates)
    person = 1
    for x, y, w, h in bounding_box_cordinates:
        cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
        # cv2.putText(frame, f'person {person}', (x, y),
        #             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
        # cv2.putText(frame, f'person {totalperson}', (x + 150, y),
        #             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
        print(weights)
        print("[INFO] detection took: {}s".format(
            (datetime.datetime.now() - start).total_seconds()))
        person += 1
        if [x,y,w,h] not in detections:
    ...
    cv2.putText(frame, f'Persons : {person - 1}',
                (40, 70), cv2.FONT_HERSHEY_DUPLEX, 0.8, (255, 0, 0), 2)
    cv2.putText(frame, f'Total Persons : {totalperson - 1}',
                (200, 70), cv2.FONT_HERSHEY_DUPLEX, 0.8, (255, 0, 0), 2)
